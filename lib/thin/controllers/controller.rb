  ...
  module Controllers  
    ...
    class Controller
      ...
      def start
        ...
        if @options[:ssl]
          server.ssl = true
          server.ssl_options = { :private_key_file => @options[:ssl_key_file], :cert_chain_file => @options[:ssl_cert_file], :verify_peer => !@options[:ssl_disable_verify] , :cipher_list => @options[:ssl_cipher_list]}
        end
